// Isolated tool module — pins the golangci-lint version so it never mixes with
// the framework's own dependency graph (the analog of PHP's per-tool
// .github/ci/<tool>/composer.json). golangci-lint is declared as a `go tool`
// and invoked from the repo root via `go tool -modfile=.github/ci/lint/go.mod`.
//
// This module is never imported, so its name is a local identifier rather than
// a real import path.
//
// NOTE: run `cd .github/ci/lint && go mod tidy` once (with network access) to
// generate go.sum and the full require graph before committing.
module valkyrja-template-lint
